function varargout = jump_plate_executable(varargin)
% JUMP_PLATE_EXECUTABLE MATLAB code for jump_plate_executable.fig
%      JUMP_PLATE_EXECUTABLE, by itself, creates a new JUMP_PLATE_EXECUTABLE or raises the existing
%      singleton*.
%
%      H = JUMP_PLATE_EXECUTABLE returns the handle to a new JUMP_PLATE_EXECUTABLE or the handle to
%      the existing singleton*.
%
%      JUMP_PLATE_EXECUTABLE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in JUMP_PLATE_EXECUTABLE.M with the given input arguments.
%
%      JUMP_PLATE_EXECUTABLE('Property','Value',...) creates a new JUMP_PLATE_EXECUTABLE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before jump_plate_executable_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to jump_plate_executable_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help jump_plate_executable

% Last Modified by GUIDE v2.5 17-Feb-2018 11:46:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @jump_plate_executable_OpeningFcn, ...
                   'gui_OutputFcn',  @jump_plate_executable_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before jump_plate_executable is made visible.
function jump_plate_executable_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to jump_plate_executable (see VARARGIN)

% Choose default command line output for jump_plate_executable
...
% UIWAIT makes jump_plate_executable wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = jump_plate_executable_OutputFcn(hObject, eventdata, handles) 
...
set(handles.jump_height_text, 'BackgroundColor', [1 0 0])
...
set(handles.jump_height_text,'String','No Data')
...
while t < (handles.total_time) %for loop that iterates for the amount of time specified. timing will need to be tweaked if a sampling interval other than one second is used.
    %t = ; %notes the time elapsed since tic
    
   
   t=toc;
    
    data_out(i,1) = toc; %puts data into an i X 4 matrix for output to CSV
    data_out(i,2) = readVoltage(handles.a,'A0');
   % data_out(i,3) = readVoltage(handles.a,'A1');
...
if i==1
    ...
    line([data_out(i-1,1) data_out(i,1)],[data_out(i-1,2) data_out(i,2)])
end
drawnow limitrate
...
%plot(data_out(:,1),data_out(:,2),'b')
%hold on
%plot(data_out(:,1),data_out(:,3),'g')
%hold off
i
...
export_name=strcat('vertical_',get(handles.player_number_edit,'String'))
cd(uigetdir)
test_info={'Player Number';'year';'month';'day';'hour';'minute';'second';'Sample Duration';'Sample Rate';'Take off Time',;'Landing Time';'Time Measured';'Jump Height (in)'}
c=fix(clock)
test_results={get(handles.player_number_edit,'String');num2str(c(1));num2str(c(2));num2str(c(3));num2str(c(4));num2str(c(5));num2str(c(6));handles.total_time;handles.sample_rate;handles.time_point_1;handles.time_point_2;handles.measured_time;handles.jump_height}
...
xlswrite(strcat(export_name,'.xlsx'),{'Time (s)' 'Signal Jump Plate (V)' 'Signal Laser (V)'},'Sheet1','E2')
...
text(handles.time_point_1+.1,handles.raw_data(out,2),'Taking off','HorizontalAlignment','right');
...
handles.jump_height=floor((accel*handles.measured_time^2)/(8)*3.38*12*10)/10
...
text(handles.time_point_2+.1,handles.raw_data(out,2),'Landing');
...
set(handles.jump_height_text,'String',num2str(handles.jump_height))
set(handles.time_point_2_text, 'BackgroundColor', [0 1 0])
set(handles.measured_time_text, 'BackgroundColor', [0 1 0])
set(handles.jump_height_text, 'BackgroundColor', [0 1 0])
